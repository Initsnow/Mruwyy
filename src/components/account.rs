...
pub fn AccountDetail() -> Element {
    ...
    if let Some(ref account_info) = status.to_owned().read().read().unwrap().login {
        let AccountInfo { name, avatar_url, .. } = account_info;
        ...
        let future: Resource<Result<(Tracks, Tracks), Box<dyn Error>>> = use_resource(|| async {
            ...
            let mut uid: u64 = 0;
            if let Some(ref account_info) = status
                .to_owned()
                .read()
                .read()
                .unwrap()
                .login
            {
                let AccountInfo { name: name_, uid: uid_, .. } = account_info;
                name_.clone_into(&mut name);
                uid = *uid_;
            }
            let lists = api.user_song_list(uid, 0, 100).await?;
            let tracks: Tracks = lists.into_iter().collect();
            let (userlists, userfavoritelists): (Tracks, Tracks) = tracks
                .vec
                .into_iter()
                .partition(|e| e.author == name);
            Ok((userlists, userfavoritelists))
        });
        rsx! {
            match &* future.read() { Some(Ok(response)) => rsx! { div { id :
            "user_info_container", img { class : "avatar", src : "{avatar_url}", } div {
            id : "user_info", h1 { "{name}" } } div { id : "background", background :
            "url({avatar_url}) center" } } div { id : "user_list_container", UserList {
            lists : response.0.to_owned() } UserFavoriteList { lists : response.1
